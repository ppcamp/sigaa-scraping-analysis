...
def _generate_competency_graphs(
    ...
    logging.debug('-'*100)
    logging.debug('Creating competency graphs')
    for competencia in competencias:
        logging.debug(f'--> {competencia}')
        ...
        for periodo in __get_period():
            ...
            while prox_periodo < 10:
                ...
                logging.debug(f'\t\t\t\tClasses: [{materias_herdeiras}]')
                # Força a saída deste loop
                break

            logging.debug('\t\tStop searching for future classes')
        ...
        logging.debug(f'Stop walking over periods for {competencia}')

        # adiciona o último nó
        grafos[competencia].add_node('Fim/Formou', period=periodo+1)
        logging.debug('Adding the last node "Fim/Formou"')
        logging.debug(f'Last value is: {last}')
        if last:
            logging.debug(f"{competencia}. ")
